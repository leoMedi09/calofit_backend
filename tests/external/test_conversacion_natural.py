...
class TestConversacionNatural:
    ...
    async def test_caso4_masa_muscular(self, setup_user):
        ...
        prog = (
            ...
            .filter(ProgresoCalorias.client_id == client.id, ProgresoCalorias.fecha == date.today())
            .first()
        )
        if not prog:
        ...
        resp = await asistente.consultar(
            mensaje="Ya pasé mi meta de calorías, ¿como algo más?", db=db, current_user=user, historial=[]
        )
    ...
    async def test_caso5_perfil_restrictivo_recomendacion(self, setup_user):
        ...
        assert "leche" not in resp_text
        assert any(
            w in resp_text for w in ["tofu", "quinua", "lenteja", "soya", "frijol", "legumbre", "garbanzo", "proteína"]
        )
