...
async def _llamar_groq_con_excepciones(
    ia_engine, prompt: str, max_tokens: int = 800, temp: float = 0.7, model: str = None
) -> str:
    import asyncio
    import httpx
    ...
    if (
        ...
        or "límite de consultas alcanzado" in raw_lower
    ):
...
def _asegurar_contexto(
    perfil, consumido: float = 0.0, quemado: float = 0.0, plan_hoy: dict = None, ctx: Optional[UserContext] = None
) -> UserContext:
    if ctx is not None:
        return ctx
    ...
    for cond in condiciones:
        ...
        if "vegano" in cond_lower:
            restricciones.append(
                "VEGANO: PROHIBIDO todo producto animal (carne, pollo, pescado, huevo, leche, queso, yogur, mantequilla, miel)."
            )
            ...
            restricciones.append(
                "DIABETES: evitar azúcares refinados, miel, dulces y alimentos de alto índice glucémico (pan blanco, arroz blanco en exceso). Preferir carbohidratos complejos y fibra."
            )
        elif "hipertensión" in cond_lower or "hipertension" in cond_lower or "hipertenso" in cond_lower:
            restricciones.append(
                "HIPERTENSIÓN: limitar sodio, sal de mesa añadida, embutidos, enlatados y alimentos ultraprocesados."
            )
        elif "lactosa" in cond_lower or "intolerancia a la lactosa" in cond_lower:
            restricciones.append(
                "INTOLERANCIA A LA LACTOSA: PROHIBIDO lácteos enteros. Permitido versiones deslactosadas o alternativas vegetales."
            )
        elif "celiaco" in cond_lower or "celíaco" in cond_lower or "gluten" in cond_lower:
            restricciones.append(
                "CELIAQUÍA / SIN GLUTEN: PROHIBIDO trigo, cebada, centeno y derivados. Solo alimentos certificados gluten-free."
            )
        else:
            restricciones.append(
                f"{cond}: Seguir las pautas de alimentación recomendadas por tu médico para esta condición."
            )
...
def _calcular_balance_meta(
    ...
    es_masa_muscular = _es_superavit_goal(objetivo) or bool(_RX_SUPERAVIT_MENSAJE.search(mensaje or ""))
    ...
    if es_masa_muscular and excedido > 0:
        ...
        advertencia_natural = "Como tu objetivo es ganar masa muscular, estar en superávit es esperado y correcto."
    elif excedido > 0:
        bloque_balance = (
        ...
        advertencia_natural = (
            f"Ya superaste tu meta de hoy por {round(excedido)} kcal — si aún quieres comer algo, mejor que sea ligero."
        )
...
Respuesta:"""
_PROMPT_RECOMENDACION_EJERCICIO = (
    _IDENTIDAD
    + """
...
async def registrar_comida_llm(
    ...
    if not goto_save:
        ...
        if _prematch_peligroso:
            ...
            logger.warning("[Registro] Guard pre-LLM: item incomestible detectado: '%s'", _item_pre)
            return {
                "success": False,
    ...
    if not goto_save:
        try:
            ...
            raw = await _llamar_groq_con_excepciones(
                ia_engine, prompt, max_tokens=700, temp=0.0, model="llama-3.3-70b-versatile"
            )
            datos = _parse_json(raw)
            if datos is None:
    ...
    if _item_peligroso:
        logger.warning(
            "[Registro] Item incomestible/peligroso detectado: '%s' en mensaje: '%s'", _item_peligroso, mensaje[:100]
        )
        return {
            "success": False,
            "tipo_detectado": "no_alimento",
    ...
    if datos.get("contiene_no_alimento"):
        logger.warning("[Registro] LLM detectó material no comestible en: '%s'", mensaje[:100])
        return {
            "success": False,
            "tipo_detectado": "no_alimento",
            "mensaje": (
    ...
    if datos.get("alimentos"):
        ...
        datos["alimentos"] = [a for a in datos["alimentos"] if a.get("es_real", True) is not False]
        if not goto_save:
            datos["alimentos"] = [
                a for a in datos["alimentos"] if _extraccion_tiene_base_textual(a.get("nombre", ""), mensaje)
            ]
            datos["alimentos"] = [
                a for a in datos["alimentos"] if not _es_solo_palabra_momento_dia(a.get("nombre", ""))
            ]
            datos["alimentos"] = [a for a in datos["alimentos"] if not _alimento_es_alucinacion(a, db)]
    ...
    if not goto_save and _items:
        ...
        if _faltantes:
            ...
            try:
                _raw_faltante = await _llamar_groq_con_excepciones(
                    ia_engine, _prompt_faltante, max_tokens=250, temp=0.0, model="llama-3.3-70b-versatile"
                )
            ...
            if _datos_faltante and _datos_faltante.get("alimentos"):
                ...
                _nuevos = [
                    a
                    for a in _datos_faltante["alimentos"]
    ...
    _match_gramos_explicitos = re.search(r"\b(\d+(?:[.,]\d+)?)\s*(?:gr|grs|gramos?|g)\b", _msg_low_porcion)
    ...
    if (
        _is_sopa
        and not _tiene_lado_solido
        and kcal > _KCAL_CAP_SOPA
        and not advertencia_cantidad
        and not advertencia_momento
    ):
...
async def registrar_ejercicio_llm(
    ...
    ejercicios_raw = [e for e in ejercicios_raw if e.get("encontrado", True) and e.get("ejercicio")]
    ejercicios_raw = [
        e for e in ejercicios_raw if _extraccion_tiene_base_textual(e.get("ejercicio", ""), mensaje, es_ejercicio=True)
    ]
...
def _construir_mensaje_natural_reco(
    ...
    kcal_txt = f"~{kcal_min:.0f} kcal" if kcal_min == kcal_max else f"entre {kcal_min:.0f} y {kcal_max:.0f} kcal"
...
async def respuesta_recomendacion_llm(
    ...
    if modo == "ejercicio":
        from app.services.rutina_service import (
            _LESIONES_SUSTITUCION,
            _detectar_lesiones,
            filtrar_lesiones_activas,
        )

        _condiciones_lista_ej = list(getattr(perfil, "medical_conditions", None) or [])
        _texto_historial_ej = " ".join(str(h.get("content", "")) for h in (historial or []))
        _lesiones_candidatas_ej = _detectar_lesiones(_condiciones_lista_ej + [mensaje or "", _texto_historial_ej])
        _lesiones_activas = filtrar_lesiones_activas(_lesiones_candidatas_ej, historial, mensaje)
        ...
        if _lesiones_activas:
            ...
            _justif_previa = "; ".join(_LESIONES_SUSTITUCION[l]["justificacion"] for l in _lesiones_activas)
            _contexto_lesion = (
        ...
        try:
            ...
            if _lesiones_activas:
                _viola_lesion = any(r in _normalizar_nombre(respuesta_ej or "") for r in _ejercicios_riesgosos)

                if _viola_lesion:
                    logger.warning("[Reco-Ejercicio] Ejercicio riesgoso para lesión detectada — reintentando")
                    _justif_lesion = "; ".join(_LESIONES_SUSTITUCION[l]["justificacion"] for l in _lesiones_activas)
                    _prompt_retry_ej = (
                    ...
                    _aun_viola_ej = any(r in _normalizar_nombre(respuesta_ej or "") for r in _ejercicios_riesgosos)
                    if _aun_viola_ej:
                        logger.warning("[Reco-Ejercicio] Reintento también riesgoso — usando fallback seguro")
    ...
    _condicion_relevante_msg = " y ".join(dict.fromkeys(c for _, c in _evitados_msg)) if _evitados_msg else None
    ...
    _estilo_evitado_momento = next(
        (e for e in _ESTILOS_NO_PERMITIDOS_MOMENTO.get(momento_reco, ()) if e in _msg_low_reco),
        None,
    )
    ...
    if db is not None:
        try:
            ...
            _excluidos_48h = [
                row[0]
                for row in db.query(HistorialRecomendacion.nombre_plato)
                .filter(
            ...
            if _tokens_dieta_reco_knn:
                _candidatos_knn = [
                    c for c in _candidatos_knn if not any(t in c["alimento"].lower() for t in _tokens_dieta_reco_knn)
                ]
        except Exception as e:
    ...
    es_vegano_reco = (
        "vegano" in dieta.lower()
        or "vegetariano" in dieta.lower()
        or "vegano" in _condiciones_str_reco
        or "vegetariano" in _condiciones_str_reco
        or "Vegano" in _condiciones_msg_reco
        or "Vegetariano" in _condiciones_msg_reco
    )
    ...
    _condiciones_sin_dieta = ", ".join(
        c.strip() for c in (condiciones or "").split(",") if c.strip().lower() not in _DIETA_NO_MEDICA
    )
    ...
    if _condiciones_sin_dieta and _condiciones_sin_dieta.lower() != "ninguna":
        ...
        try:
            ...
            _restricciones_raw = await _llamar_groq_con_excepciones(ia_engine, _prompt_med, max_tokens=200, temp=0.0)
    ...
    if _ya_vistos:
        ...
        _ya_sugeridos_txt = f"PLATOS YA RECOMENDADOS (NO repetir): {', '.join(_vistos_unicos[:8])}.\n\n"
    ...
    _restr_momento = (_RESTRICCIONES_MOMENTO_VEGANO if es_vegano_reco else _RESTRICCIONES_MOMENTO_RECO).get(
        momento_reco, ""
    )

    _contexto_dieta = restriccion_dieta_reco or (f"Objetivo: {objetivo}." if not es_vegano_reco else "")
    _restricciones_extra = " | ".join(
        filter(
            None,
            [
    ...
    def _extraer_texto_y_platos(resp: str):
        ...
        if platos_line:
            for part in platos_line[len("PLATOS:") :].strip().split("|"):
                ...
                if m:
                    platos.append(
                        (m.group(1).strip(), float(m.group(2)), float(m.group(3)), float(m.group(4)), float(m.group(5)))
                    )
                else:
        ...
        if platos and (not texto or not _nombres_en_texto):
            partes = [f"{n} (~{k:.0f} kcal)" if k else n for n, k, *_ in platos]
    ...
    try:
        ...
        if _tokens_dieta_check and any(_linea_viola_dieta(l) for l in _texto_usuario.split("\n")):
            ...
            respuesta_llm_reco = await _llamar_groq_con_excepciones(ia_engine, _prompt_retry, max_tokens=250, temp=0.3)
            _texto_usuario, _platos_data = _extraer_texto_y_platos(respuesta_llm_reco)
    ...
    return f"{_prefijo}{_texto_usuario}".strip() or "No pude generar sugerencias en este momento. Intenta de nuevo."
...
def _lesion_mencionada_sin_tipo(mensaje: str, historial: list) -> bool:
    ...
    if historial:
        _turnos_usuario = [str(h.get("content", "")) for h in historial if h.get("role") == "user"]
...
def _filtrar_resultado_chat(texto: str, tema: str) -> str:
    ...
    _kw_evitar = _TEMA_NUTRICION_KW if tema == "ejercicio" else (_TEMA_EJERCICIO_KW if tema == "nutricion" else ())
...
async def respuesta_chat_llm(
    ...
    hist_txt = (
        "\n".join(f"{'Usuario' if m['role'] == 'user' else 'Asistente'}: {m['content'][:120]}" for m in historial[-4:])
    ...
    if _tema_chat == "general" and _es_peticion_ambigua(mensaje):
        return "¿Sobre qué tema necesitas ayuda? Puedo ayudarte con nutrición, ejercicio o seguimiento de tu progreso."
    ...
    _pregunta_uso_app = ("como uso" in _m_norm_app or "como funciona" in _m_norm_app) and (
        "app" in _m_norm_app or "esto" in _m_norm_app or "aplicacion" in _m_norm_app
    )
    ...
    if _RE_PUEDO_COMER.match(mensaje.strip()):
        ...
        otras = [c for c in _conds_raw if not any(k in c.lower() for k in ("vegano", "vegetariano", "diabetes"))]
        if otras:
        ...
        if _nombre_escaped:
            _resultado_perm = _re_puedo.sub(rf"^{_nombre_escaped},\s*", "", _resultado_perm, flags=_re_puedo.IGNORECASE)
            if _resultado_perm:
    ...
    if _es_consulta_kcal:
        ...
        if d_macros and d_macros.get("alimentos"):
            ...
            _unidad_r = (
                "ml" if "ml" in mensaje.lower() or "jugo" in mensaje.lower() or "leche" in mensaje.lower() else "g"
            )
            _partes_r = []
            for _val, _nom in ((p_r, "proteína"), (c_r, "carbohidratos"), (g_r, "grasa")):
                _partes_r.append(f"casi nada de {_nom}" if _val < 0.5 else f"{_formato_num(_val)}g de {_nom}")
            return (
    ...
    try:
            ...
            resultado = await validate_and_retry(
                ...
                construir_prompt_retry=lambda: (
                    ...
                    + (
                        ...
                        if _repite_mensaje_usuario(resultado, mensaje)
                        else ""
                    )
                    + (
        ...
        _lesiones_candidatas_chat = _detectar_lesiones(_condiciones_lista_chat + [mensaje or "", _texto_hist_chat])
        _lesiones_activas_chat = filtrar_lesiones_activas(_lesiones_candidatas_chat, historial, mensaje)
        if _lesiones_activas_chat:
            ...
            if any(r in _normalizar_nombre(resultado) for r in _riesgosos_chat):
                logger.warning("[Chat] Ejercicio riesgoso para lesión detectada en conversación libre — reintentando")
                _justif_chat = "; ".join(_LESIONES_SUSTITUCION[l]["justificacion"] for l in _lesiones_activas_chat)
                _prompt_retry_chat = (
    ...
    if _es_masa_muscular:
        ...
        if any(k in _msg_low for k in ("comer", "como", "meta", "caloria", "caloría", "algo")):
            ...
            if not any(w in _res_low for w in ("ganar", "músculo", "musculo", "masa", "superávit", "superavit")):
                _es_advertencia = any(
                    w in _res_low for w in ("excediste", "pasaste", "excedido", "evita", "modera", "cuidado", "alerta")
                )
                _nota_masa = (
...
def _recortar_respuesta_chat(texto: str, mensaje_usuario: str, max_oraciones: int = 3) -> str:
    ...
    _pidio_consejo = any(p in mensaje_usuario.lower() for p in ("consejo", "ayuda", "ayudame", "ayúdame"))
...
def _filtrar_componentes_de_plato_compuesto(alimentos: list[dict]) -> list[dict]:
    ...
    def _palabras_clave(nombre: str) -> set[str]:
        return {w for w in _normalizar_nombre(nombre or "").split() if w not in _CONECTORES_PLATO_COMPUESTO}
...
def _filtrar_contenedor_generico_con_ingredientes(alimentos: list[dict], mensaje: str = "") -> list[dict]:
    ...
    for idx_c, a_c, palabras_c in potenciales_contenedores:
        keywords_c = {w for w in palabras_c[1:] if w not in _STOPWORDS_BASE_TEXTUAL and len(w) > 2}
        ...
        if container_word in msg_lower:
            ...
            for i, a in enumerate(alimentos):
                ...
                other_words = {
                    w
                    for w in _palabras_nombre(a.get("nombre", ""))
...
def _es_solo_palabra_momento_dia(nombre: str) -> bool:
    ...
    palabra de momento)."""
    palabras = {w for w in _normalizar_nombre(nombre or "").split() if w not in _CONECTORES_PLATO_COMPUESTO}
...
def _existe_en_bd_alimentos(db, nombre: str) -> bool:
    ...
    palabras = [p for p in nombre_norm.split() if len(p) >= 3 and p not in _CONECTORES_PLATO_COMPUESTO]
    if not palabras:
        return True
    try:
        condiciones = " OR ".join(
            f"unaccent(lower(nombre_normalizado)) ~* ('(^| )' || unaccent(lower(:p{i})) || '( |$)')"
...
def _extraccion_tiene_base_textual(
    ...
    palabras = [p for p in _nombre_norm.split() if len(p) > 3 and p not in _STOPWORDS_BASE_TEXTUAL]
    if not palabras:
        return True
...
def _palabras_faltantes_en_extraccion(mensaje: str, alimentos: list[dict]) -> list[str]:
    ...
    palabras_extraidas = set(_normalizar_nombre(" ".join(a.get("nombre", "") for a in alimentos)).split())
    ...
    palabras_msg = [
        p
        for p in (t.strip(_PUNTUACION_BORDE) for t in _normalizar_nombre(mensaje or "").split())
        if len(p) >= 4
    ...
    def _ya_cubierta_difusa(p: str) -> bool:
        return any(
            _difflib_completitud.SequenceMatcher(None, p, w).ratio() >= 0.7 for w in palabras_extraidas if len(w) >= 4
        )

    _candidatas = [
        p
        for p in palabras_msg
        if _singular(p) not in _extraidas_singular and not _ya_cubierta_como_substring(p) and not _ya_cubierta_difusa(p)
    ]
    if _candidatas and _candidatas[-1] == "causa":
        _palabras_msg_full = [w.rstrip(_PUNTUACION_BORDE) for w in _normalizar_nombre(mensaje or "").split()]
...
def _detectar_conflicto_dieta(
    nombres: list,
    diet_type: str,
    condiciones_medicas: list | None = None,
) -> str | None:
    ...
    prohibidos = _ANIMAL_VEGANO if "vegano" in dieta else (_ANIMAL_VEGETARIANO if "vegetariano" in dieta else set())
    if prohibidos:
