...
async def _descomponer_plato_llm(nombre_plato: str) -> List[dict]:
    ...
    try:
        ...
        prompt = (
            ...
            f"  * FRITO / FRITA / FRITAS / FRITURA → usa SIEMPRE la variante frita del ingrediente:\n"
            f"    'Yucas Fritas' → 'Yuca Frita' (NO 'Yuca Cocida' ni 'Yuca Sancochada')\n"
            f"    'Papas Fritas' → 'Papa Frita' (NO 'Papa Cocida')\n"
            f"    'Pollo Frito' → 'Pollo Frito' (NO 'Pechuga Cocida')\n"
            f"    'Pescado Frito' → 'Pescado Blanco Frito' (NO 'Pescado Blanco Cocido')\n"
            f"    REGLA GENERAL: si el nombre contiene 'frit', el ingrediente principal es su variante FRITA.\n"
            f"    NUNCA reemplaces 'frito/frita' por 'cocido/cocida/sancochado/sancochada'.\n"
