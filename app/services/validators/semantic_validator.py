...
_CATEGORIAS: Dict[str, List[str]] = {
    ...
    "mariscos": ["camarón", "langostino", "cangrejo", "pulpo", "calamar", "mejillón", "concha", "choro"],
    ...
    "carbohidratos": ["arroz", "papa", "camote", "pasta", "pan", "quinua", "maíz", "yuca", "trigo"],
    "proteínas": [
        "pollo",
        "pato",
        "pavo",
    ...
    "gluten": ["trigo", "harina", "pasta", "pan", "cebada", "centeno", "avena", "sémola", "gluten"],
    "frutas": ["manzana", "naranja", "plátano", "limón", "lima", "mango", "uva", "piña", "pera", "durazno", "fresa"],
    "condimentos": [
        "sal",
        "pimienta",
        "comino",
...
_COMBOS_INCOHERENTES: List[tuple] = [
    (
        "tostada",
        "plátano",
        "Tostada de plátano no es una preparación estándar — considera 'plátano asado' o 'tostones'",
    ),
...
class SemanticValidator(BaseValidator):
    ...
    def validar(self, datos: Dict[str, Any]) -> ValidationResult:
        ...
        if tipo and tipo in _REGLAS:
            ...
            for req in regla["requiere"]:
                if not any(req in ing for ing in nombres):
                    advertencias.append(f"{tipo.capitalize()} habitualmente lleva '{req}'")
                    confianza -= 5
            for proh in regla["prohíbe"]:
                if any(proh in ing for ing in nombres):
                    errores.append(f"{tipo.capitalize()} no debe llevar '{proh}': {regla['descripcion']}")
                    confianza -= 20
        ...
        for patron_plato, ingrediente_incompat, mensaje in _COMBOS_INCOHERENTES:
            ...
            ing_match = ingrediente_incompat in nombres_ings_lower or ingrediente_incompat in nombre_plato_lower
        ...
        if ficticios:
            advertencias.append(f"Ingredientes no reconocidos como alimentos reales: {', '.join(sorted(ficticios))}")
            confianza -= 60
        ...
        if len(ingredientes) > 10:
            advertencias.append(f"Plato con {len(ingredientes)} ingredientes — verifica que sea coherente")
            confianza -= 5

        if len(cats) == 1:
            advertencias.append(f"Todos los ingredientes caen en una sola categoría: {list(cats)[0]}")
    ...
    def _restricciones_cliente(self, client_id: int, ingredientes: List[str]) -> Dict[str, List[str]]:
        try:
