...
class NutricionService:
    ...
    def _buscar_en_sqlite(self, nombre_busqueda: str) -> Optional[dict]:
        ...
        try:
            ...
            with sqlite3.connect(db_path) as conn:
                ...
                if not row:
                    cursor.execute(
                        "SELECT * FROM alimentos WHERE nombre LIKE ? ORDER BY calorias DESC LIMIT 1",
                        (f"%{nombre_busqueda}%",),
                    )
                    row = cursor.fetchone()
        ...
        for k, v in self._datos_nutricionales.items():
            ...
            if pos == -1:
                if k in nombre_clean:
                    pos = 0
                elif nombre_clean in k:
                    pos = k.find(nombre_clean)
                else:
                    continue
    ...
    def _normalizar_ram(self, item_raw: dict) -> dict:
        ...
        cal = (
            item_raw.get("calorias")
            or item_raw.get("calorias_100g")
            or item_raw.get("Energía (kcal)")
            or item_raw.get("Energía \n(kcal)")
            or 0
        )
        prot = (
            item_raw.get("proteinas")
            or item_raw.get("proteina_100g")
            or item_raw.get("Proteína \n(g)")
            or item_raw.get("Proteína (g)")
            or 0
        )
        carb = (
            item_raw.get("carbohidratos")
            or item_raw.get("carbohindratos_100g")
            or item_raw.get("Carbohidratos \n(g)")
            or item_raw.get("Carbohidratos totales (g)")
            or 0
        )
        gras = (
            item_raw.get("grasas")
            or item_raw.get("grasas_100g")
            or item_raw.get("Grasa \n(g)")
            or item_raw.get("Grasa total (g)")
            or 0
        )
