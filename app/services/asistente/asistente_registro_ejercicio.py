...
_UNIDADES_STANDALONE = {k: v for k, v in _UNIDADES_MAP.items() if k not in ("un", "uno", "una")}

_RE_NUMERO_COMPUESTO = re.compile(
    r"(?i)\b("
    + "|".join(sorted(_CENTENAS_MAP, key=len, reverse=True))
    + r")(?:\s+("
    + "|".join(sorted(_DECENAS_MAP, key=len, reverse=True))
    + r"))?(?:\s+y\s+("
    + "|".join(sorted(_UNIDADES_MAP, key=len, reverse=True))
    ...
    + r"))?(?:\s+(y\s+medi[ao]|y\s+cuarto|y\s+tres\s+cuartos))?\b"
    + r"|\b("
    + "|".join(sorted(_UNIDADES_STANDALONE, key=len, reverse=True))
    + r")(?:\s+(y\s+medi[ao]|y\s+cuarto|y\s+tres\s+cuartos))?\b"
)
...
class RegistroEjercicioHandler:
    ...
    def registrar_workout_log(
        ...
        """
        cal = round(ejercicios_service.calcular_calorias(met, peso_corporal_kg, duracion_min), 1)
    ...
    def get_workout_logs(self, client_id: int, db: Session, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        return [
            {
                ...
                "peso_kg": r[3],
                "created_at": str(r[4]),
                "calorias_quemadas": r[5],
                "session_duration_min": r[6],
                "intensity": r[7],
            }
            for r in rows
        ]
