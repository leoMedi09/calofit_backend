...
from app.services.asistente.asistente_modos import (
    ...
    REGISTRAR_EJERCICIO,
    REGISTRAR_NUTRICION,
    RX_CORREGIR_REGISTRO,
    _VERBOS_IMPERATIVOS_REGISTRO,
    resolver_modo_funcion,
)
...
def _build_response(perfil, intencion, tipo_pregunta, meta, consumido, quemado, plan_hoy, mensaje_txt):
    ...
    return {
        ...
        "respuesta_estructurada": {
            "intent": intencion,
            "texto_conversacional": mensaje_txt,
            "secciones": [],
        },
    }
    ...
    async def consultar(
        ...
        if modo_funcion == REGISTRAR_NUTRICION:
            _solo_verbo = (
                bool(
                    re.search(
                        r"^acabo\s+de\s+(?:comer|cenar|desayunar|almorzar|tomar|beber)\s*$", msg_limpio, re.IGNORECASE
                    )
                )
            ...
            if not _com.get("success") and RX_CORREGIR_REGISTRO.search(msg_limpio) and historial:
                _turnos_usuario_prev = [h.get("content", "") for h in historial if h.get("role") == "user"]
        ...
        return _build_response(
            perfil, "INFO", "OTRO", calorias_meta, consumo_real, quemadas_real, plan_hoy_data, _texto_chat
        )
        ...
        if modo_funcion == REGISTRAR_NUTRICION:
            resp_est["secciones"] = [s for s in (resp_est.get("secciones") or []) if s.get("tipo") != "comida"]
        ...
        if _tokens_guard and modo_funcion == "recomendar_nutricion":
            ...
            def _contiene_prohibido(sec: dict) -> bool:
                texto = (sec.get("nombre", "") + " " + " ".join(sec.get("ingredientes", []))).lower()
                return any(t in texto for t in _tokens_guard)

            secciones_antes = resp_est.get("secciones") or []
            resp_est["secciones"] = [
                s for s in secciones_antes if not (s.get("tipo") == "comida" and _contiene_prohibido(s))
            ]
            n_filtradas = len(secciones_antes) - len(resp_est.get("secciones", []))
            if n_filtradas:
                logger.warning(
                    "[DietGuard] %d sección(es) eliminadas por contener ingredientes prohibidos (%s)",
                    n_filtradas,
                    list(_tokens_guard)[:5],
                )
        ...
        if _restantes_actual <= 50 and modo_funcion == "recomendar_nutricion" and not _msg_insiste:
            ...
            if isinstance(resp_est.get("respuesta_estructurada"), dict):
                resp_est["respuesta_estructurada"]["texto_conversacional"] = resp_est["texto_conversacional"]
        ...
        if modo_funcion == OTRO:
            ...
            resp_est["secciones"] = [
                s for s in (resp_est.get("secciones") or []) if s.get("tipo") not in ("ejercicio", "rutina")
            ]
            import re as _re_m
