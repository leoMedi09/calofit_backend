...
class RecomendadorPlatosConfiables:
    ...
    def _calcular_score(
        ...
        # Bonus por proteína cuando hay déficit proteico
        if d_prot > 5 and prot >= 15:
            score += 10
        elif d_prot > 5 and prot < 5:
            score -= 10

        # Penalizar platos con macros extremas (muy grasosos o muy simples)
        if gras > kcal * 0.6:  # más del 60% de kcal de grasas
            score -= 15
        if carb < 2 and prot < 5:
            score -= 20
    ...
    def _generar_y_validar_nuevos_platos(
        ...
        try:
            ...
            prompt = (
                ...
                f"{deficit_carb:.0f}g carbohidratos, {deficit_grasas:.0f}g grasas.\n"
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as pool:
                    future = pool.submit(asyncio.run, llm.completar(prompt=prompt, max_tokens=1000))
                    respuesta = future.result(timeout=20)
            else:
                respuesta = loop.run_until_complete(llm.completar(prompt=prompt, max_tokens=1000))
            ...
            nuevos = []
            propuestas = json.loads(m.group(0))
            ...
            for prop in propuestas:
                ...
                if resultado.exito and resultado.confianza_global >= _MIN_CONFIANZA:
                    kcal = sum(i.macros_totales.calorias for i in resultado.ingredientes if i.macros_totales)
                    prot = sum(i.macros_totales.proteina for i in resultado.ingredientes if i.macros_totales)
                    carb = sum(i.macros_totales.carbohidratos for i in resultado.ingredientes if i.macros_totales)
                    gras = sum(i.macros_totales.grasas for i in resultado.ingredientes if i.macros_totales)
                    ...
                    for i in resultado.ingredientes:
                        _kcal_i = i.macros_totales.calorias if i.macros_totales else 0
